...
export class ScienceLibrary extends Library {
  books: Book[];
  ...
  static createLibrary(books: Book[], members: Member[]): ScienceLibrary {
    const librarians = members.filter(m => m instanceof Librarian);
    const normalMembers = members.filter(m => !(m instanceof Librarian));
    return new ScienceLibrary(books, normalMembers, librarians);
  }
